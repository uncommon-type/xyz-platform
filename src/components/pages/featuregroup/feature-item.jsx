...
const FeatureItem = ({ title = '', subtitle = '', content = '', className = '' }) => (
  <li className="card flow flex flex-col w-[min(100%,21.125rem)]">
    <div className="card__stats flow offset relative">
      <h2 className="relative font-poppins-light font-[275] text-3xl tracking-tightest lining-nums proportional-nums bg-medium-gradient text-transparent bg-clip-text">
        {title}
      </h2>
      <span className={clsx(baseClasses, className)} />
      <p className="text-xl font-roboto-light leading-4.5 tracking-widest">{subtitle}</p>
    </div>
    ...
    <div className="card__details flow flex flex-col flex-grow">
      ...
      <p className="flex items-start text-grey-94 font-roboto-light font-light text-md leading-snug tracking-wide">
        {content}
      </p>
    </div>
  </li>
